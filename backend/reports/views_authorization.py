...
class SignatoryAuthorizationViewSet(viewsets.ModelViewSet):
    ...
    def get_permissions(self):
        ...
        """
        print(f"🔍 GET_PERMISSIONS called for action: {self.action}")
        ...
        if self.action in ['request_authorization', 'my_authorizations', 'my_requests']:
            print(f"🔍 Returning IsAuthenticated for {self.action}")
            return [IsAuthenticated()]
        # Admin-only actions
        elif self.action in ['pending_requests', 'approve_request', 'reject_request']:
            print(f"🔍 Returning Admin permissions for {self.action}")
        ...
        # Public actions (signature setup via token)
            ...
            print(f"🔍 Returning AllowAny for {self.action}")
            return [AllowAny()]
        # Default: authenticated users only
        print(f"🔍 Returning default IsAuthenticated for {self.action}")
        return [IsAuthenticated()]
    
    def dispatch(self, request, *args, **kwargs):
        """Override dispatch to log all requests"""
        print(f"🔍 DISPATCH - User: {request.user}")
    ...
    @action(detail=False, methods=['post'], url_path='request', permission_classes=[AllowAny])
    ...
    def request_authorization(self, request):
        ...
        try:
            with open('method_called.txt', 'w', encoding='utf-8') as f:
                ...
                f.write(f"User: {request.user}\n")
                f.write(f"User authenticated: {request.user.is_authenticated}\n")
    ...
    def save_signature(self, request, token=None):
        ...
        try:
            ...
            print(f"🖊️ Saving signature for {authorization.signatory_name}...")
            ...
            print(f"✅ Authorization updated in database: ID={authorization.id}, signature_created=True")
            
            # Verify database save
            authorization.refresh_from_db()
            if not authorization.signature_created:
                raise Exception("Failed to save signature_created flag to database")
            
            print(f"✅ Database verification passed: signature_created={authorization.signature_created}")
            ...
            # Send notification emails to admin and requestor
            try:
                print(f"📧 Sending signature completion notifications...")
...
NPC Reporting System
    ...
    def _notify_signature_completed(self, authorization):
        """Send email notification when signature is completed"""
        try:
            from django.contrib.auth.models import User
            
            print(f"📧 Preparing signature completion notifications for {authorization.signatory_name}...")
            
            # Get the authorization request to find the original email
            ...
            # 1. Send to the requestor/user
            user_email = auth_request.email if auth_request else authorization.user.email
            if user_email:
                recipients.append(user_email)
                print(f"📧 Will notify requestor: {user_email}")
            ...
            if admin_emails:
                print(f"📧 Will notify {len(admin_emails)} admin(s): {', '.join(admin_emails)}")
            
            if not recipients:
                print("⚠️ No recipients found for signature completion notification")
                return
            ...
            # Extract last name for greeting
            signatory_parts = authorization.signatory_name.split()
            if len(signatory_parts) > 1:
                last_name = signatory_parts[-1]
                if last_name.upper() in ['JR.', 'JR', 'SR.', 'SR', 'III', 'II']:
                    last_name = signatory_parts[-2] if len(signatory_parts) > 2 else signatory_parts[0]
            else:
                last_name = authorization.signatory_name
            
            subject = f'✅ E-Signature Completed - {authorization.signatory_name}'
            message = f"""
Dear Team,

Great news! The e-signature for {authorization.signatory_name} has been successfully created and is now ready to use.

E-Signature Details:
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Signatory Name: {authorization.signatory_name}
User: {authorization.user.get_full_name() or authorization.user.username}
Status: ✅ ACTIVE
Signature Created: {timezone.now().strftime('%B %d, %Y at %I:%M %p')}
2FA Security: {'Enabled' if authorization.requires_2fa else 'Disabled'}
Authorization Expires: {authorization.expiry_date.strftime('%B %d, %Y') if authorization.expiry_date else 'Never'}
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
✅ {authorization.signatory_name} can now sign reports using this e-signature
...
For Requestor ({authorization.user.get_full_name() or authorization.user.username}):
You can now use your e-signature to sign reports in the NPC Reporting System. Your signature will automatically appear when you are selected as a signatory for any report.

For Administrators:
The e-signature has been verified and saved to the database. You can view the signature in the admin panel under Signatory Authorizations.
...
This is an automated notification. Please do not reply to this email.
            ...
            print(f"📧 Sending signature completion notification to {len(recipients)} recipient(s)...")
            ...
            print(f"✅ Signature completion notification sent successfully to: {', '.join(recipients)}")
            
            # Log the notification
            AuditLogger.log_user_action(
                user=authorization.user,
                action='SIGNATURE_COMPLETION_NOTIFIED',
                description=f'Sent signature completion notification for {authorization.signatory_name} to {len(recipients)} recipient(s)',
                model_name='SignatoryAuthorization',
                object_id=authorization.id,
                category='e_signature',
                severity='LOW',
